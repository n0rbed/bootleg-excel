...
def graph_polynomial(data_list, degree):
    ...
    equation = ''.join(f"{v}x^{len(z) - (i+1)} + " for i, v in enumerate(z))
    equation = equation[:-3]
    return equation
...
def graph_normal_distribution(data_list, yx_or_both, x_title, y_title):
    ...
    if yx_or_both == 'both':
        ...
        returns['mean'] = f'μx = {meanx}, μy = {meany}'
        returns['stdev'] = f'σx = {round(stdevx, 3)}, σy = {round(stdevy, 3)}'
